 ...
 desired_capacity          = var.autoscaling_group_desired_capacity
  ...
  tag {
    ...
    tag {
      ...
      value =  var.instance_name == null ? upper(join("-",[(local.standard_name["environment"] == "DRE" ? "AZO" : "AZV"), join("",[local.standard_name["server_type"] ,substr(local.standard_name["layer"],0,1),local.standard_name["os_version"]]), join("",["${upper(local.standard_name["environment"])=="DRE" || upper(local.standard_name["environment"])=="DBG" ? substr(local.standard_name["environment"],1,1) : substr(local.standard_name["environment"],0,1) }","${local.standard_namea["application_id"]}",local.standard_name["deploy_method"] != "" ? substr(local.standard_name["deploy_method"],0,1): ""]) ])) : var.instance_name
      propagate_at_launch  = true
  }
