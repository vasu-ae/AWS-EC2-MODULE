...
name = var.autosacling_group_name == null ? lower(join("-" , [ join("-",[(var.environment == "DRE" ? "AZO" : "AZV"), join("",["AUS", var.server_type])]) , join("",["${upper(var.environment)=="DRE" || upper(var.environment)=="DBG" ? substr(var.environment,1,1) : substr(var.environment,0,1) }","${var.application_id}"]) ])) : var.eks_autoscaling_group_creation ? upper(join("-" , [ join("",[(var.environment == "DRE" ? "AZO" : "AZV"),"EKS","CLU", join("",["AUS", var.server_type])]) , join("",["${upper(var.environment)=="DRE" || upper(var.environment)=="DBG" ? substr(var.environment,1,1) : substr(var.environment,0,1) }","${var.application_id}"]) ])) : var.autosacling_group_name
